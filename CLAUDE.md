...
| [`docs/plano-video-final.md`](docs/plano-video-final.md) | Ao retomar o **mini-ciclo «O vídeo final»** — o rabo do Ciclo 3, nascido do veredito do dono em 02/09: a Máquina termina em **UM vídeo montado, sem Spark**. Fases 0 a 5, tudo 0 ⚡. **Rascunho aguardando aprovação.** |
| [`docs/plano-fix-produto.md`](docs/plano-fix-produto.md) | Ao retomar a **Frente A′ · o `fix:` do produto** — por que *nome não é foto*, o que a investigação de 06/09 achou (o slot da Máquina existe e o fio é **inerte**; o compilador já sabe compilar produto; o campo `produto` do schema não tem descrição; o aviso existe no bloco errado), as quatro fases de 0 ⚡, as duas provas do dono a **90 ⚡** e as cinco perguntas que só ele responde. **Plano em aprovação.** |
...
8. **Fluxo de encerramento de toda tarefa:** reescrever [`docs/ESTADO.md`](docs/ESTADO.md) → conferir que `git grep -n "SAI ANTES DO COMMIT" -- src/ supabase/` volta **vazio** → apresentar resumo enxuto do que foi feito → rodar `git status` e mostrar a lista de arquivos a commitar → pedir ok ao Jorge → só então fazer commit e push. Se aparecer qualquer arquivo fora do escopo declarado da tarefa, **parar e avisar antes de commitar**.
   ...
   **Toda leitura de número do canvas AFIRMA que a aba foi pintada — e recusa a leitura se não foi** *(mecanismo do Jorge, 06/09/2026)*. O instrumento é `scratchpad\harness\medir-canvas.js`, e ele não devolve número nenhum sem uma de **duas** portas: **(1) pintura ao vivo** — `document.visibilityState === "visible"` **e** um `requestAnimationFrame` que **dispara**; ou **(2) pintura anterior** — **nenhum node em `visibility: hidden`**, que é a marca do React Flow para *"ainda não medi este node"*. **E toda afirmação sobre o store lê o store**, nunca a tela.

   **A segunda porta não é frouxidão — é o conserto de um falso vermelho medido.** Só a (1) recusava o caso bom: um `screenshot` no meio do `browser_batch` pinta a aba, o React Flow mede e desenha as 20 arestas, e mesmo assim, quando o JS roda, a aba já voltou a `hidden` e o rAF não dispara. *Trava que recusa o caso bom é trava que se aprende a burlar.* Vermelho→verde no mesmo gesto: sem `screenshot`, *"27 de 27 nodes por medir · LEITURA RECUSADA"*; com `screenshot`, `27 nodes · 20 arestas · 0 por medir`.

   **Por que é mecanismo e não frase:** a armadilha já estava escrita — *«NADA de canvas vale em aba escondida»*, 18/08/2026 — e mordeu de novo assim mesmo. Em aba não pintada o Chrome não faz layout, o React Flow não mede os nodes e **não desenha aresta nenhuma**, sem avisar. Isso custou três semanas: virou «incidente dos vínculos», uma trava de servidor e uma fase inteira do plano, tudo a partir de *27 nodes e ZERO arestas* lidos numa aba que o navegador nunca pintou — com o store cheio o tempo todo. **Uma frase num arquivo não é uma trava**, do mesmo jeito que o `SAI ANTES DO COMMIT` não era antes de virar `git grep`. → [`docs/decisoes.md`](docs/decisoes.md), 06/09/2026
