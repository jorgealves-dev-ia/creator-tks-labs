...
| [`docs/nodes-geracao.md`](docs/nodes-geracao.md) | Ao mexer nos nodes do canvas — bloco Gerar Imagem, node Resultado, menções `@` no prompt, referências anexadas e sua galeria, presets de formato por canal, ou a compilação de canvas. Especificação v1 aprovada com as decisões N1–N5. |
