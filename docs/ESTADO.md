...
**Última reescrita:** 07/09/2026, no fechamento da **Frente A′**.

> # ✅ ÁRVORE LIMPA. Nada esperando validação, nada não commitado.
>
> A frente **A′ · o `fix:` do produto** foi **provada pelo dono e fechada** em 07/09.
> O mini-ciclo «O vídeo final» está fechado desde 06/09. **O próximo gesto é novo trabalho.**
...
| 3 | **Backlog de UI do percurso de 07/09 — três itens, 0 ⚡**, vistos pelo dono enquanto fazia a metade dele: **(a)** o **scroll dentro do diálogo de cena rola o CANVAS** em vez do diálogo; **(b)** o botão do portão diz **«Gerar de novo» em cena que nunca teve imagem** — *"de novo"* é uma afirmação sobre o passado, e é falsa na primeira vez: deve ser **«Gerar»** na primeira e **«Gerar de novo»** só após falha ou com imagem existente; **(c)** a mensagem de recusa deve dizer **«bloqueada pelo filtro do Google»** — quem lê precisa saber que a recusa é do provedor e que reescrever a frase é o caminho. | Claude |
...
A árvore está limpa e em produção. **Três candidatos, todos 0 ⚡**, na ordem em que o
próprio ESTADO os classifica:

1. **O produto diz o que é** (item 1) — o endereço deixado pela frente que acabou de fechar,
   com a causa já medida e dois caminhos escritos. **É o mais barato de todos: nenhum dos
   dois caminhos toca no motor de geração.** ⚠️ *Provar isto de verdade custa uma imagem
   (75 ⚡) — a fidelidade só se vê gerando.*
...
3. **Os três itens de UI** (item 3) — pequenos, independentes, e o (c) fecha o que a recusa
   de 07/09 mostrou.
