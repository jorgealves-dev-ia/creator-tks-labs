...
- Seeds: **Google Nano Banana Pro** (padrão, `is_default` para image_gen), GPT Image (OpenAI), Grok Imagine (xAI) — slugs e preços oficiais conferidos na documentação na implementação; preço inicial em Sparks = chute educado com margem sobre o custo oficial, calibrável com dados reais (o padrão da casa).
